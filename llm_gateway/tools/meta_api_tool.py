...
class APIMetaTool(BaseTool):
    """Tool for dynamically exposing FastAPI endpoints via MCP."""

    tool_name = "api_meta_tool"
    description = "Tool for dynamically exposing FastAPI endpoints via MCP."

    def __init__(self, mcp_server):
        """Initialize the API Meta-Tool.

        Args:
            mcp_server: MCP server instance
        """
        super().__init__(mcp_server)
        self.registered_apis = {}  # Maps API names to their information
        self.generated_tools = {}  # Maps tool names to their generated functions
    ...
    async def register_api(
        ...
        if not base_url:
            ...
            try:
                ...
                self.logger.info(f"Using base_url: {base_url} (derived from openapi_url)")
        ...
        # Fetch OpenAPI spec
        self.logger.info(f"Fetching OpenAPI spec from {openapi_url}")
        start_time = time.time()
        openapi_spec = await fetch_openapi_spec(openapi_url, timeout, headers)

        # Extract endpoint information
        endpoints = extract_endpoint_info(openapi_spec)
        self.logger.info(f"Extracted {len(endpoints)} endpoints from OpenAPI spec")

        # Generate and register tools for each endpoint
        registered_tools = []
        generated_code = {}  # For debugging, store the generated code
        ...
        for endpoint in endpoints:
            ...
            try:
                ...
                bound_method = generated_func.__get__(self, self.__class__)

                # Register with MCP server
                self.mcp.tool(name=tool_name)(bound_method)

                # Store the generated tool
                self.generated_tools[tool_name] = bound_method
                ...
                self.logger.error(f"Failed to register tool {tool_name}: {str(e)}", exc_info=True)
                if "tool_code" in locals():
                    self.logger.error(f"Generated code that failed:\n{tool_code}")
    ...
    @with_tool_metrics
    @with_error_handling
    async def list_registered_apis(self) -> Dict[str, Any]:
        ...
        for api_name, api_info in self.registered_apis.items():
    ...
    @with_tool_metrics
    @with_error_handling
    async def get_api_details(self, api_name: str) -> Dict[str, Any]:
    ...
    @with_tool_metrics
    @with_error_handling
    async def unregister_api(self, api_name: str) -> Dict[str, Any]:
        ...
        for tool_name in tools:
            try:
                ...
                self.logger.error(f"Failed to unregister tool {tool_name}: {str(e)}", exc_info=True)

        # Remove API from registered APIs
        del self.registered_apis[api_name]

        self.logger.success(f"API {api_name} unregistered with {len(tools)} tools")
    ...
    async def call_dynamic_tool(
        ...
        This is a convenience method for calling tools that were dynamically
        registered from an OpenAPI spec.

        Args:
            tool_name: Name of the tool to call (e.g., "example_api_get_users")
            inputs: Dictionary of inputs to pass to the tool
            auth_token: Optional authentication token to include in requests
        ...
        tool_func = self.generated_tools[tool_name]

        # Determine which API this tool belongs to
        api_name = None
        ...
        if not api_name:
            self.logger.warning(f"Could not determine which API {tool_name} belongs to")

        # Add auth_token to inputs if specified and the API has an auth_header
        if auth_token and api_name and self.registered_apis[api_name].get("auth_header"):
            inputs["auth_token"] = auth_token

        # Add timeout to inputs
        ...
        self.logger.info(f"Called dynamic tool {tool_name} in {processing_time:.4f}s")
        return result

    @with_tool_metrics
    @with_error_handling
    async def refresh_api(
        self, api_name: str, update_base_url: Optional[str] = None, timeout: float = 30.0
    ) -> Dict[str, Any]:
    ...
    @with_tool_metrics
    @with_error_handling
    async def list_available_tools(self, include_source_code: bool = False) -> Dict[str, Any]:
        """Lists all dynamically registered tools.

        Args:
            include_source_code: Whether to include the source code of each tool
                ...
                "tools": [
                    {
                        ...
                        "path": "/users",
                        "summary": "Get all users",
                        "source_code": "..." (if include_source_code is True)
                    },
                    ...
                ],
                "tools_count": 12
...
def register_api_meta_tools(mcp_server):
    ...
    """
    # Initialize the API Meta-Tool
    api_meta_tool = APIMetaTool(mcp_server)

    # Register tools with MCP server
    mcp_server.tool(name="register_api")(api_meta_tool.register_api)
    mcp_server.tool(name="list_registered_apis")(api_meta_tool.list_registered_apis)
    mcp_server.tool(name="get_api_details")(api_meta_tool.get_api_details)
    mcp_server.tool(name="unregister_api")(api_meta_tool.unregister_api)
    mcp_server.tool(name="call_dynamic_tool")(api_meta_tool.call_dynamic_tool)
    mcp_server.tool(name="refresh_api")(api_meta_tool.refresh_api)
    mcp_server.tool(name="get_tool_details")(api_meta_tool.get_tool_details)
    mcp_server.tool(name="list_available_tools")(api_meta_tool.list_available_tools)

    return api_meta_tool
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        api_meta_tool = register_api_meta_tools(mcp_server)

        # Process commands
        if args.register and args.url:
            result = await api_meta_tool.register_api(
                api_name=args.register, openapi_url=args.url, base_url=args.base_url
            )
            ...
            result = await api_meta_tool.get_api_details(args.details)
            ...
            result = await api_meta_tool.unregister_api(args.unregister)
            print(f"Unregistered API {args.unregister} with {result['tools_count']} tools")
        elif args.refresh:
            result = await api_meta_tool.refresh_api(
                api_name=args.refresh, update_base_url=args.base_url
            )
            print(
