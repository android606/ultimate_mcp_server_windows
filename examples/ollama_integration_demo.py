...
async def compare_ollama_models(tracker: CostTracker):
    ...
    try:
        ...
        if not models_to_compare:
            ...
            console.print("[red]Selected models not found. Use 'ollama pull llama3.2' to download models first.[/red]")
            return
...
async def demonstrate_system_prompt(tracker: CostTracker):
    ...
    try:
        ...
        # Use llama3.2 (ensure it's available)
        model = "llama3.2"
        available_models = await provider.list_models()
        model_names = [m["id"] for m in available_models]
        
        if model not in model_names:
            logger.warning(f"Model {model} not available, please run 'ollama pull llama3.2'", emoji_key="warning")
            console.print("[yellow]Model not found. Please run 'ollama pull llama3.2' to download it.[/yellow]")
            return
...
async def explore_ollama_models():
    ...
    try:
        ...
        if not models:
            ...
            console.print("Example: [green]ollama pull llama3.2[/green]")
            return
...
async def main():
    ...
    console.print(Panel(
        ...
        "- Pull models with [green]ollama pull llama3.2[/green] or similar commands",
